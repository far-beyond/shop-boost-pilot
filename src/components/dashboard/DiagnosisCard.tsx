          ...
          {item.status === "completed" && (
            <div className="flex gap-2 flex-wrap">
              <Link to={`/diagnosis/${item.id}`}>
                <Button size="sm" className="gap-1.5 text-[11px] sm:text-xs h-7 sm:h-8">
                  <Eye className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                  レポート
                </Button>
              </Link>
              <Link to={`/promo/${item.id}`}>
                <Button size="sm" variant="outline" className="gap-1.5 text-[11px] sm:text-xs h-7 sm:h-8">
